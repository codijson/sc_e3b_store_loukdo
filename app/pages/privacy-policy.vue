...
import { PrivacyPolicyDocument, CookiesPolicyDocument } from "~/core/terms/PrivacyPolicyDocument";
      ...
      <template v-else>
        <header class="text-center" v-reveal>
          ...
          <p class="mt-3 text-sm text-ink-400">
            ...
            {{ t("privacy.effective", { date: effectiveDate }) }}
          </p>
        </header>
        ...
        <BaseCard class="mt-8 divide-y divide-transparent">
          <TermsSection
            v-for="section in privacyDocument.sections"
            :key="section.id"
            :section="section"
            :company="companyName"
          />
        ...
        <!-- Cookies Policy -->
        <h2
          class="mt-14 text-center font-display font-extrabold leading-[1.08] text-primary dark:text-primary-medium text-xl sm:text-2xl"
        >
          {{ t("cookies.title") }}
        </h2>
        ...
        <BaseCard class="mt-8 divide-y divide-transparent">
          <TermsSection
            v-for="section in cookiesDocument.sections"
            :key="section.id"
            :section="section"
            :company="companyName"
          />
