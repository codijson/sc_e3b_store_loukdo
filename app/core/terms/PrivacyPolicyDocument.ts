/**
 * PrivacyPolicyDocument / CookiesPolicyDocument
 * ----------------------------------------------
 * Structured description of the "Privacy & Cookies Policy" page. The
 * source design renders this as two visually separate documents (each
 * with its own independent numbering, starting back at "1.") stacked on
 * one page under a single title — so this file exports two document
 * classes rather than one, and the page renders two BaseCards.
 *
 * See the doc comment on TermsDocument.ts for why documents are modelled
 * as sections of subsections rather than hard-coded markup.
 */
...
export class PrivacyPolicyDocument {
  ...
  static build(): PrivacyPolicyDocument {
...
export class CookiesPolicyDocument {
  private constructor(public readonly sections: TermsSection[]) {}

  static build(): CookiesPolicyDocument {
